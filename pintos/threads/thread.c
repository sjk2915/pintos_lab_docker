...
void thread_update_priority(struct thread *t, void *aux UNUSED)
{
    if (!thread_mlfqs)
    {
        ...
        if (!list_empty(&(t->donors)))
        {
            struct thread *max_donor =
                list_entry(list_max(&t->donors, cmp_priority, ASC), struct thread, donation_elem);
    ...
    else
    {
        ...
        int prio = PRI_MAX - fp_to_int_round(div_fp_int(t->recent_cpu, 4)) - (t->nice * 2);
        if (prio > PRI_MAX)
            prio = PRI_MAX;
        if (prio < PRI_MIN)
            prio = PRI_MIN;
...
static void thread_update_recent_cpu(struct thread *t, void *aux)
{
    ...
    t->recent_cpu = add_fp_int(mult_fp(*(fp *)aux, t->recent_cpu), t->nice);
}
...
void thread_mlfqs_update_recent_cpu(void)
{
    ...
    int coef = div_fp(mult_fp_int(load_avg, 2), add_fp_int(mult_fp_int(load_avg, 2), 1));
