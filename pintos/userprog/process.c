...
__do_fork(void *aux)
{
	 ...
	 * TODO:       부모의 자원 복제가 성공할 때까지 부모는 fork()에서
	 * TODO:       반환하면 안 됩니다. */
...
int process_wait(tid_t child_tid UNUSED)
{
    ...
    /* 자식이 끝날 때까지 기다림 */
    while (true) {
        /* 여기서는 단순히 yield로 다른 스레드에게 CPU를 양보 */
        thread_yield();
        
        /* 실제로는 자식 프로세스의 상태를 확인해야 하지만
           지금은 무한 대기하지 않도록 임시 구현 */
