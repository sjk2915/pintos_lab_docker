...
{
	// 잘못된 fd 접근
	if (fd < 0 || fd >= t->fdt_size) return -1;
	// 없는 fd 접근
...
{
	struct thread *cur = thread_current();
	if (check_fd(cur, fd) == -1) return -1; 
	if (IS_STDIO(cur->fdt[fd])) return -1;
...
static int sys_read (int fd, void *buffer, unsigned size)
{
	...
	// 콘솔출력에 읽기 금지
	else if (cur->fdt[fd] == STDOUT_FDNO) return -1;
...
static int sys_write (int fd, const void *buffer, unsigned size)
{
	...
	// 콘솔입력에 쓰기 금지
	if (cur->fdt[fd] == STDIN_FDNO) return -1;
...
{
	struct thread *cur = thread_current();
	if (check_fd(cur, fd) == -1) return;
	if (IS_STDIO(cur->fdt[fd])) return;
...
static int sys_dup2(int oldfd, int newfd)
{
	struct thread *cur = thread_current();
	// fd 검사
	if (check_fd(cur, oldfd) == -1) return -1;
	if (newfd < 0) return -1;
	if (oldfd == newfd) return newfd;
	
	// 원하는 숫자보다 사이즈가 작으면 확장시도
	while (cur->fdt_size < newfd)
		if (extend_fdt(cur) == -1) return -1;
	// 열려있으면 닫기
